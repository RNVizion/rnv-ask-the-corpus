...
**A `require` pattern carries the noun it counts, not just the figure.** `\b(nine|9)\b` looks like it guards the project count and does not: inside `res-count`'s own five-chunk window that word arrives from rnv-color-mcp's tool count, the text transformer's format count, and four times over in an essay about the count being wrong. An answer reading *"ten projects, built on nine deterministic tools"* scored a hit. Requiring the figure adjacent to its noun is what makes the pattern assert a claim instead of detecting a token — which is the same failure `claim` was added to fix, recurring one level up inside the fix. Keep the gap small: it is sized to the phrasings the corpus actually produces (`nine projects`, `Nine shipped projects`, `5,000+ tests`), and a wider one lets an unrelated noun drift back into range.
